...
class ExpenseController extends GetxController {
  ...
  Future<void> fetchCategories() async {
    try {
      ...
      if (response.data['status'] == true) {
        final List data = response.data['data'];
        categories.value = data.map((e) => ExpenseCategoryModel.fromJson(e)).toList();
      }
    } catch (e) {
  ...
  List<ExpenseModel> get filteredExpenses {
    ...
    return expenses.where((exp) => exp.category.toString() == filterCategoryId.value).toList();
  }

  double get totalExpense {
    return filteredExpenses.fold(0.0, (sum, exp) => sum + double.parse(exp.amount));
  }
  ...
  Future<void> deleteCategory(int id, String name) async {
    final categoryExpenses = expenses.where((exp) => exp.category.toString() == id.toString()).toList();

    if (categoryExpenses.isNotEmpty) {
      double catTotal = categoryExpenses.fold(0.0, (sum, exp) => sum + double.parse(exp.amount));
        ...
        AlertDialog(
          ...
          content: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              const Text("All associated expenses will be deleted first!", style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
            ],
          ),
          actions: [
            ...
            TextButton(
              ...
              child: const Text("Yes, delete anyway!", style: TextStyle(color: Colors.red)),
            ),
          ],
        ),
      );
